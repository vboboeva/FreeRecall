...
{		
	...
	int stimuli[2*(2*L+2)+1];
	int background[2*(2*L+2)+1];
	int Tupdates[2*(2*L+2)+1];
	int cues[2*(2*L+2)+1];
	...
	stimuli[2*L]=0;
	background[2*L]=0;
	Tupdates[2*L]=T_recall;
	cues[2*L]=-1;

	stimuli[2*L+1]=0;
	background[2*L+1]=0; /*1*/
	Tupdates[2*L+1]=T_end; 
	cues[2*L+1]=-1;

	/* LIST 2 */

	c=L;
	for(int y=2*L+2;y<2*(2*L)+2;y++)
	{
		if (y%2==0) 
		{
			stimuli[y]=0;
	...
	stimuli[2*(2*L)+2]=0;
	background[2*(2*L)+2]=0;
	Tupdates[2*(2*L)+2]=T_recall;
	cues[2*(2*L)+2]=-1;

	stimuli[2*(2*L)+3]=0;
	background[2*(2*L)+3]=0; /*1*/
	Tupdates[2*(2*L)+3]=T_end; 
	cues[2*(2*L)+3]=-1;

	stimuli[2*(2*L)+4]=0;
	background[2*(2*L)+4]=0; /*1*/
	Tupdates[2*(2*L)+4]=2*T_recall; 
	cues[2*(2*L)+4]=-1;	
		...
		for(int ttt=0;ttt<Tupdates[i];ttt++) 
		{
			...
			for(int j=0;j<N;j++)
			{
				...
				if((n%snap)==0)
				{
					...
					/*RECALL*/
					if(stimuli[i]==0 && background[i]==0 && Tupdates[i]==2*T_recall && cues[i]==-1)
					{
						/*printf("recall list\n");*/
...
char command[0x1000];
snprintf(outdir, sizeof(outdir), "output_p%d_L%d_alpha%.4f_beta%.4f_gamma%.4f_zeta%.4f_taupre%.1f_kappa%.3f_pattset%d", p, L, alpha, beta, gamma, zeta, tau_pre, kappa, pattset);

...
void run_net(int pattset, int set)
{		
...
char command[0x1000];
snprintf(outdir, sizeof(outdir), "output_N%d_a%.2f_p%d_L%d_dJ%.4f_kappa%.1f_set%d", N, a, p, L, dJ, kappa, pattset);
...
char filename[0x100];
snprintf(filename, sizeof(filename), "input/pattern_set%d_N%d_a%.2f_p%d", pattset, N, a, p);
